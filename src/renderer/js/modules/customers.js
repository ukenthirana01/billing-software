    ...
    <div class="card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>#</th><th>Name</th><th>Phone</th><th>Email</th><th>GST No</th><th>State</th><th>Credit Limit</th><th>Opening Balance</th><th>Actions</th></tr></thead>
          <tbody>${list.map((c,i)=>`
            <tr>
              <td>${i+1}</td>
              ...
              <td class="actions">
                <button class="btn btn-sm btn-info" onclick="showCustomerStatementModal(${c.id})"><i class="bi bi-file-earmark-pdf"></i></button>
                <button class="btn btn-sm btn-secondary" onclick="editCustomer(${c.id})">✏ Edit</button>
                <button class="btn btn-sm btn-danger admin-only" onclick="deleteCustomer(${c.id})">🗑</button>
               </td>
...
function customerForm(c={}) {
  ...
  return `<div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">Email</label><input class="form-control" id="cf-email" type="email" value="${c.email||''}" placeholder="customer@email.com" /></div>
