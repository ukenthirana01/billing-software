...
function renderDelTable(){document.getElementById('del-tbody').innerHTML=delItems.map((i,idx)=>`<tr><td>${idx+1}</td><td>${window.escapeHtml(i.product_name)}</td><td><input type="number" value="${i.qty}" onchange="updateDelItem(${idx},'qty',this.value)" style="width:60px" /></td><td>${window.escapeHtml(i.unit)}</td><td><button class="btn btn-sm btn-danger" onclick="removeDelItem(${idx})">✕</button></td></tr>`).join('');}
window.updateDelItem=(i,f,v)=>{delItems[i][f]=Math.max(0,parseFloat(v)||0);renderDelTable();};
