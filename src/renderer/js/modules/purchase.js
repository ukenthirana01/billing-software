        ...
        <div class="card mb-4">
          <div class="card-header">
            ...
            <div style="display:flex; gap:8px;">
              <button class="btn btn-sm btn-primary" onclick="launchBillScanner()"><i class="bi bi-camera"></i> Scan Bill</button>
...
window.launchBillScanner = async function() {
  try {
    const { openScanner } = await import('./bill-scanner.js');
    openScanner((extractedItems) => {
      extractedItems.forEach(item => {
        // Try to map to an existing product
        const matchedProd = purchaseProducts.find(p => p.name.toLowerCase() === item.product_name.toLowerCase() || (p.code && p.code.toLowerCase() === item.product_name.toLowerCase()));
        
        let pId = null; 
        let isService = window.currentAppMode === 'service' ? 1 : 0;
        let unit = 'Nos';
        let hsn = '';

        if (matchedProd) {
          pId = matchedProd.id;
          isService = matchedProd.is_service || 0;
          unit = matchedProd.unit || 'Nos';
          hsn = matchedProd.hsn_code || '';
        }
        ...
        purchItems.push({
          product_id: pId,
          product_name: item.product_name + (matchedProd ? '' : ' (New)'),
          hsn_code: hsn,
          qty: item.qty,
          unit: unit,
          price: item.price,
      ...
      recalcPurchase();
      toast(`${extractedItems.length} items successfully imported`, 'success');
    });
  } catch (err) {
    console.error('Failed to load scanner:', err);
    toast('Scanner module could not be loaded.', 'error');
  }
};
            ...
            <tr>
              ...
              <td>
                <div class="actions">
                  ...
                  <button class="btn btn-sm btn-danger admin-only" onclick="deletePurchase(${b.id})"><i class="bi bi-trash-fill"></i></button>
                </div>
              </td>
  ...
  box.innerHTML = `
    <div class="modal-header">
      ...
      <div class="flex gap-2">
        ...
        <button class="btn btn-sm btn-danger admin-only" onclick="deletePurchase(${bill.id});document.getElementById('modal-overlay').classList.add('hidden')"><i class="bi bi-trash-fill"></i></button>
