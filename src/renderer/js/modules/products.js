    ...
    <div class="card">
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>#</th><th>Name</th><th>Barcode</th><th>Category</th><th>HSN</th><th>GST%</th><th>Purchase ₹</th><th>Selling ₹</th><th>MRP ₹</th><th>Unit</th><th>Stock</th><th>Expiry</th><th>Actions</th></tr></thead>
          <tbody>${list.map((p,i)=>`
            <tr>
              <td>${i+1}</td>
              ...
              <td>${p.mrp ? fmtCur(p.mrp) : '-'}</td>
              <td>${p.unit}</td>
              <td><span class="badge ${p.stock<=p.min_stock&&p.min_stock>0?'badge-danger':'badge-success'}">${p.stock}</span></td>
              <td>${p.expiry_date ? `<span class="badge ${new Date(p.expiry_date) < new Date() ? 'badge-danger' : 'badge-success'}">${p.expiry_date}</span>` : '-'}</td>
              <td class="actions">
                <button class="btn btn-sm btn-secondary" onclick="editProduct(${p.id})">✏</button>
                <button class="btn btn-sm btn-danger admin-only" onclick="deleteProduct(${p.id})">🗑</button>
              </td>
            </tr>`).join('') || '<tr><td colspan="13" class="text-center text-muted" style="padding:30px">No products found</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>`;
...
window.downloadCSVTemplate = function() {
  const header = 'name,code,hsn_code,gst_percent,purchase_price,selling_price,mrp,unit,stock,min_stock,is_service,batch_no,mfg_date,expiry_date';
  const example = 'Sample Product,PRD001,1234,18,100,150,180,Nos,50,5,0,BATCH001,2025-01-01,2027-12-31';
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const product = {
        ...
        category_id:    null,
        batch_no:       String(row[col('batch_no')] || '').trim().slice(0, 50),
        mfg_date:       String(row[col('mfg_date')] || '').trim().slice(0, 10),
        expiry_date:    String(row[col('expiry_date')] || '').trim().slice(0, 10),
      };
...
function productForm(p={}) {
  ...
  return `<div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">Product Code/Barcode</label>
      <div style="display:flex;gap:4px">
        <input class="form-control" id="pf-code" value="${p.code||''}" style="flex:1" />
        <button class="btn btn-sm btn-secondary" type="button" onclick="autoGenerateBarcode()" title="Auto-generate a unique barcode">⚡ Auto</button>
      </div>
    </div>
    ...
    <div class="form-group"><label class="form-label">MRP (₹)</label><input class="form-control" id="pf-mrp" type="number" step="0.01" value="${p.mrp||0}" /></div>
    ...
    <div class="form-group"><label class="form-label">Batch No.</label><input class="form-control" id="pf-batch" value="${p.batch_no||''}" placeholder="e.g. BATCH-001" /></div>
    <div class="form-group"><label class="form-label">Mfg. Date</label><input class="form-control" id="pf-mfg" type="date" value="${p.mfg_date||''}" /></div>
    <div class="form-group"><label class="form-label">Expiry Date</label><input class="form-control" id="pf-expiry" type="date" value="${p.expiry_date||''}" /></div>
...
function getProductData() {
  return {
    ...
    batch_no: document.getElementById('pf-batch').value.trim(),
    mfg_date: document.getElementById('pf-mfg').value,
    expiry_date: document.getElementById('pf-expiry').value,
  };
}

// Auto-generate a unique barcode code
window.autoGenerateBarcode = function() {
  const existingCodes = allProducts.map(p => p.code).filter(Boolean);
  let num = allProducts.length + 1;
  let code;
  do {
    code = 'MSB-' + String(num).padStart(6, '0');
    num++;
  } while (existingCodes.includes(code));
  const el = document.getElementById('pf-code');
  if (el) { el.value = code; }
  toast(`Generated barcode: ${code}`, 'success');
};
...
window.manageCategories = function() {
  const list = allCategories.map(c=>`<div class="flex items-center justify-between" style="padding:6px 0;border-bottom:1px solid var(--gray-100)"><span>${c.name}</span><button class="btn btn-sm btn-danger admin-only" onclick="deleteCategory(${c.id})">🗑</button></div>`).join('');
